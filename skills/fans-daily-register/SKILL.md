...
登记入口是 **工时管理系统**，不是 TAPD。Cursor / Codex 共用本技能。Codex 可点名 `$fans-daily-register`。
...
| 邮箱 | `fanzongling@up366.com`（无密码，`userName` 即邮箱） |
| 身份 | 樊宗领，`userId` 769846，`isLeader=0`，`roles=[]`（开发，不是负责人） |

环境由页面域名决定：`up366demo.cn` → API 也是 `*.up366demo.cn`。
...
`login` 不传参数则用上表邮箱。`daily` 默认当天。响应 `code === 0` 才算成功；业务数据在 `data`。
